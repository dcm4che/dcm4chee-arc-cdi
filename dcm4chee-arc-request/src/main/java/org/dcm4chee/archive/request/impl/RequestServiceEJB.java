...
package org.dcm4chee.archive.request.impl;
...
import org.dcm4chee.archive.request.EntityAlreadyExistsException;
import org.dcm4chee.archive.request.PatientMismatchException;
 ...
 */
@Stateless
public class RequestServiceEJB implements RequestService {
    ...
    private ScheduledProcedureStep findOrCreateScheduleProcedureStep(
        ...
        try {
            ...
            Patient spsPatient = sps.getRequestedProcedure()
                    .getServiceRequest().getVisit().getPatient();
            if (patient.getPk() != spsPatient.getPk())
                throw new PatientMismatchException(sps, patient, spsPatient);
            return sps;
    ...
    private Visit getVisit( Attributes attrs,
        ...
        try {
            ...
            if (patient.getPk() != visitPatient.getPk())
                throw new PatientMismatchException(visit, patient, visitPatient);
            return visit;
        } catch (NoResultException e) {
            return newVisit(attrs, patient, issuerOfAdmissionID, filter);
        }
    ...
    private RequestedProcedure findOrCreateRequestedProcedure(
        ...
        try {
            ...
            Patient rpPatient = rp.getServiceRequest().getVisit().getPatient();
            if (patient.getPk() != rpPatient.getPk())
                throw new PatientMismatchException(rp, patient, rpPatient);
            return rp;
        } catch (NoResultException e) {
    ...
    private ServiceRequest getServiceRequest(
        ...
        try {
            ...
            Patient requestPatient = request.getVisit().getPatient();
            if (patient.getPk() != requestPatient.getPk())
                throw new PatientMismatchException(request, patient, requestPatient);
            return request;
        } catch (NoResultException e) {
