...
public enum WadoAttributesCache {
    ...
    public Attributes getAttributes(QueryService service, Device device,
            Long seriesPk) {

        QueryParam queryParam = device
                .getDeviceExtension(ArchiveDeviceExtension.class)
                .getQueryParam();
        
        long staleTimeout = this.staleTimeout;
        if (staleTimeout <= 0)
            return service.getSeriesAttributes(seriesPk,queryParam);
        synchronized (this) {
            ...
            entry = new CacheEntry(service.getSeriesAttributes(seriesPk,queryParam), now);
