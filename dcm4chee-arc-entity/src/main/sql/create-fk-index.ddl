create index FK318FE31937EDB1AA on content_item (instance_fk);
create index FK318FE31970C135AA on content_item (code_fk);
create index FK318FE3199F40BC4C on content_item (name_fk);
create index FKD42DBF5037EDB1AA on file_ref (instance_fk);
create index FKD42DBF50206F5C8A on file_ref (filesystem_fk);
create index FKA2455AABE9B3E742 on filesystem (next_fk);
create index FK211694958151AFEA on instance (series_fk);
create index FK2116949540F8410A on instance (reject_code_fk);
create index FK211694954DC50E6B on instance (srcode_fk);
create index FK2116949585AF69D8 on instance (dicomattrs_fk);
create index FK333EE69DC28D5C on mpps (drcode_fk);
create index FK333EE6A511AE1E on mpps (patient_fk);
create index FK333EE685AF69D8 on mpps (dicomattrs_fk);
create index FK8F9D3D30A511AE1E on mwl_item (patient_fk);
create index FK8F9D3D3085AF69D8 on mwl_item (dicomattrs_fk);
create index FK8F9D3D30E53AEEC8 on mwl_item (perf_phys_name_fk);
create index FKD0D3EB05206840B on patient (merge_fk);
create index FKD0D3EB05E7945C3 on patient (pat_name_fk);
create index FKD0D3EB0585AF69D8 on patient (dicomattrs_fk);
create index FK8523EC95A511AE1E on patient_id (patient_fk);
create index FK8523EC959E0B30AA on patient_id (issuer_fk);
create index FK68FB577E4DDF450 on qc_instance_history (qc_series_history_fk);
create index FKFD935C99C2EE0096 on qc_series_history (qc_study_history_fk);
create index FKFD935C9985AF69D8 on qc_series_history (dicomattrs_fk);
create index FK154398B16C9B9292 on qc_study_history (qc_action_history_fk);
create index FK154398B185AF69D8 on qc_study_history (dicomattrs_fk);
create index FK22A324EB85AF69D8 on qc_update_history (dicomattrs_fk);
create index FK22A324EB390B582C on qc_update_history (qc_update_history_fk);
create index FKEA5CC19493CF1065 on rel_instance_file_ref (file_ref_fk);
create index FKEA5CC19437EDB1AA on rel_instance_file_ref (instance_fk);
create index FK268C10558B0E8FE9 on rel_linked_patient_id (patient_id_fk);
create index FK268C1055A511AE1E on rel_linked_patient_id (patient_fk);
create index FK2EF025C1E344D73A on rel_study_pcode (pcode_fk);
create index FK2EF025C14BDB761E on rel_study_pcode (study_fk);
create index FKCA01FE77B729CB1 on series (inst_code_fk);
create index FKCA01FE774BDB761E on series (study_fk);
create index FKCA01FE7785AF69D8 on series (dicomattrs_fk);
create index FKCA01FE77E53AEEC8 on series (perf_phys_name_fk);
create index FK530E8CA38151AFEA on series_query_attrs (series_fk);
create index FKE38CD2D6C45E7AAD on series_req (accno_issuer_fk);
create index FKE38CD2D68151AFEA on series_req (series_fk);
create index FKE38CD2D633B55733 on series_req (req_phys_name_fk);
create index FKA3E90A0A7665E75 on soundex_code (person_name_fk);
create index FK786E2A3CF8FD7F43 on sps_station_aet (mwl_item_fk);
create index FK68B0DC9C45E7AAD on study (accno_issuer_fk);
create index FK68B0DC97F2DAD5E on study (ref_phys_name_fk);
create index FK68B0DC9A511AE1E on study (patient_fk);
create index FK68B0DC985AF69D8 on study (dicomattrs_fk);
create index FK1D7130F54BDB761E on study_query_attrs (study_fk);
create index FKC9DB73DC37EDB1AA on verify_observer (instance_fk);
create index FKC9DB73DC661F04F6 on verify_observer (observer_name_fk);
