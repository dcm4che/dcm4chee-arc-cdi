...
public class GetStudyTestForIT {
    ...
    public static WebArchive createDeployment() {
        ...
        war.addClass(GetStudyTestForIT.class);
    ...
    public void testLoadInstance() throws Exception {
        ...
        List<Path> paths = service.getStudy(studyUID);
        int i = 0;
        for (Iterator iterator = paths.iterator(); iterator.hasNext();i++) {
            Path path = (Path) iterator.next();
            Path dest = Paths.get(destBase+(studyUID+"."+i+".dcm"));
            Files.copy(path, dest);            
        }

        log.info("written " + (i+1) + " files to " + destBase);
    }
