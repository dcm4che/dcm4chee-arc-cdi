...
public class PropertyMatchPatientSelector implements PatientSelector {
    ...
    public Patient select(List<Patient> patients,
        ...
        // check other ids over the one used to match (transforms the OR in the match to and AND)
        // criteria to apply demographic match (i.e. no demog if id+issuer matches)
        // add replace with regular expression to all strings (use String.replace(reg,""))
