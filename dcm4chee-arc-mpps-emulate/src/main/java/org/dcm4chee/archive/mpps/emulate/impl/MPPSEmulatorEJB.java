...
public class MPPSEmulatorEJB {
    ...
    public void scheduleMPPSEmulation(String sourceAET, String studyInstanceUID,
        ...
        try {
            ...
            LOG.debug("Update scheduled MPPS Emulation for Study[iuid={}] received from {}",
            ...
            entity.setEmulationTime(emulationTime);
            em.persist(entity);
            LOG.info("Schedule MPPS Emulation for Study[iuid={}] received from {}",
                    studyInstanceUID, sourceAET);
        }
    }
    ...
    public EmulationResult emulateNextPerformedProcedureStep(Device device,
        ...
        List<MPPSEmulate> resultList = em.createNamedQuery(
                    MPPSEmulate.FIND_READY_TO_EMULATE, MPPSEmulate.class)
                .setMaxResults(1)
                .getResultList();
        ...
        ApplicationEntity ae = device.getApplicationEntity(
                emulate.getEmulatorAET());
        if (ae == null) {
            throw new ArchiveConfigurationException(
                    "Unknown MPPS Emulator AET: " + emulate.getEmulatorAET());
        }
        MPPS mpps = emulatePerformedProcedureStep(
                ae,
                emulate.getSourceAET(),
                emulate.getStudyInstanceUID(),
                mppsService);
        em.remove(emulate);
    ...
    public MPPS emulatePerformedProcedureStep(ApplicationEntity ae,
        ...
        List<Series> series = em.createNamedQuery(
                Series.FIND_BY_STUDY_INSTANCE_UID_AND_SOURCE_AET, Series.class)
            .setParameter(1, studyInstanceUID)
            .setParameter(2, sourceAET)
            .getResultList();
        if (series.isEmpty())
        ...
        MPPS mpps = mppsService.createPerformedProcedureStep(arcAE, mppsIUID ,
                createMPPS(series),
                series.get(0).getStudy().getPatient(), mppsService);
    ...
    private void updateMPPSReferences(String mppsIUID, List<Series> series,
        ...
        for (Series ser : series) {
            ...
            serAttrs.newSequence(Tag.ReferencedPerformedProcedureStepSequence, 1)
                .add(mppsRef);
    ...
    private Attributes createMPPS(List<Series> series) {
