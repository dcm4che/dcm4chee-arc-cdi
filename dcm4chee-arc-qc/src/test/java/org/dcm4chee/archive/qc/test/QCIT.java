...
public class QCIT {
    ...
    public void testBUpdateStudyAttrs() throws Exception {
        ...
        QCEvent event = qcManager.updateDicomObject(
                archDevExt, QCUpdateScope.STUDY, attrs);
        ...
        assertTrue(procCodes.get(0).getCodeValue().equalsIgnoreCase("PROC_CODE_1"));
        ...
        assertTrue(issuer.getLocalNamespaceEntityID()
                .equalsIgnoreCase(issuerOfAccessionNumberItem.getString(
                        Tag.LocalNamespaceEntityID)));
    ...
    public void testCUpdateStudyAttrsAndLinkToNextUpdateHistory() throws Exception {
        ...
        attrs2.setString(Tag.StudyInstanceUID, VR.UI, study.getStudyInstanceUID());
        QCEvent event2 = qcManager.updateDicomObject(
                archDevExt, QCUpdateScope.STUDY, attrs2);
        ...
        QCUpdateHistory prevHistoryNode = (QCUpdateHistory) query.getSingleResult();
    ...
    public void testDUpdateSeriesAttrs() throws Exception {
        ...
        QCEvent event = qcManager.updateDicomObject(
                archDevExt, QCUpdateScope.SERIES, attrs);
        ...
        utx.commit();
        assertTrue(reqAttrs.get(0).getRequestedProcedureID().equalsIgnoreCase("P-9915"));
        ...
        assertTrue(!series.getAttributes().getSequence(Tag.RequestAttributesSequence)
                .isEmpty());
    }
    ...
    public void testEUpdateInstanceAttrs() throws Exception {
        ...
        QCEvent event = qcManager.updateDicomObject(
                archDevExt, QCUpdateScope.INSTANCE, attrs);
        ...
        assertTrue(!instanceAttributes.getSequence(Tag.VerifyingObserverSequence)
                .isEmpty());
    ...
    public void testFmergeStudiesUpdateAccessionNumberUpdateBodyPartExamined()
        ...
        QCInstanceHistory prevInstForKO = new QCInstanceHistory(
        ...
        //check moved
        assertTrue(instances.get(0).getSeries().getStudy().getStudyInstanceUID()
                .equalsIgnoreCase("3.3.3.2"));
      ...
      //check attributes update applied
        ...
        //series
        assertTrue(newKO.getSeries().getOldSeriesUID().equalsIgnoreCase("2.2.2.100"));
        //current updated
        //instance
        ...
        checkTwoDocsReferenceEachOtherInIdenticalSeq(instances.get(0).getAttributes(),
                thirdParty.get(0).getAttributes());
    }
    ...
    public void testGSplitTargetStudyExistsNoEnrichAllReferencedInstancesMoved()
      ...
      //test split all from series ( tests all KO references moved case)
        ...
        String[] instanceSOPUID = {newIMG1.getCurrentUID(), newIMG2.getCurrentUID(),
                newKO.getCurrentUID()};
        ...
        assertTrue(instances.get(1).getSeries().getStudy().getStudyInstanceUID()
                .equalsIgnoreCase("STUDY2"));
        assertTrue(instances.get(2).getSeries().getStudy().getStudyInstanceUID()
                .equalsIgnoreCase("STUDY2"));

        //same new series for instances from same old series
        assertTrue(instances.get(0).getSeries().getSeriesInstanceUID()
                .equalsIgnoreCase(instances.get(1).getSeries().getSeriesInstanceUID()));
        //different series for KO
        assertFalse(instances.get(0).getSeries().getSeriesInstanceUID()
                .equalsIgnoreCase(instances.get(2).getSeries().getSeriesInstanceUID()));
        ...
        assertTrue(newIMG1.getSeries().getOldSeriesUID().equalsIgnoreCase("SERIES2"));
        assertTrue(newIMG2.getSeries().getOldSeriesUID().equalsIgnoreCase("SERIES2"));
        assertTrue(newKO.getSeries().getOldSeriesUID().equalsIgnoreCase("SERIES1"));
        ...
        //is last item
        assertTrue(newIMG1.getCurrentUID().equalsIgnoreCase(newIMG1.getNextUID()));
        
        //instance 2
        //is last item
        assertTrue(newIMG2.getCurrentUID().equalsIgnoreCase(newIMG2.getNextUID()));
        ...
        //identical document sequence updated
        checkTwoDocsReferenceEachOtherInIdenticalSeq(instances.get(2).getAttributes(),
                thirdParty.get(0).getAttributes());
    }
    ...
    public void testHSplitTargetStudyNotExistsEnrichSomeReferencedInstancesMovedNewStudy() 
      ...
      //test split some from series ( tests some KO references moved case, implies a clone)
        ...
        String[] instanceSOPUID = {newIMG1.getCurrentUID(), newKO.getCurrentUID()};
        ...
        AttributesBlob seriesHistoryBlob = newIMG1.getSeries().getUpdatedAttributesBlob();

        assertTrue(studyHistoryBlob!=null);
        assertTrue(seriesHistoryBlob!=null);
        assertTrue(studyHistoryBlob.getAttributes().getString(Tag.AccessionNumber)
                .equalsIgnoreCase("A12345"));
        //series
        assertTrue(seriesHistoryBlob.getAttributes().getString(
                Tag.BodyPartExamined).equalsIgnoreCase("BRAIN"));
        ...
        //test all three KOs reference each other in the identical document sequence
        allReferencedInIdenticalDocumentSequence(matches.get(0),instances.get(1)
                ,thirdParty.get(0));
    ...
    private boolean allReferencedInIdenticalDocumentSequence(Instance instance,
        ...
        && checkTwoDocsReferenceEachOtherInIdenticalSeq(attrs3,attrs)
        && checkTwoDocsReferenceEachOtherInIdenticalSeq(attrs3,attrs2));
    }

    private boolean checkTwoDocsReferenceEachOtherInIdenticalSeq(Attributes attrs,
            Attributes attrs2) {
        int result=0;
        ...
        for(Attributes item : seq1) {
            if(itemContainsReferencedSOPID(item,attrs2.getString(Tag.SOPInstanceUID)))
                result++;
        }
        for(Attributes item : seq2) {
            if(itemContainsReferencedSOPID(item, attrs.getString(Tag.SOPInstanceUID)))
                result++;
        }
        if(result==2)
            return true;
    ...
    private boolean itemContainsReferencedSOPID(Attributes item, String string) {
        ...
        for(Attributes seriesItem : item.getSequence(Tag.ReferencedSeriesSequence)) {
            for(Attributes sopItem : seriesItem.getSequence(Tag.ReferencedSOPSequence)) {
    ...
    private void initSplitOrSegmentData() throws NotSupportedException, SystemException,
    ...
    private QCInstanceHistory getInstanceHistoryByOldUID(String uid) {
        Query query = em.createNamedQuery(
                QCInstanceHistory.FIND_BY_OLD_UID);
