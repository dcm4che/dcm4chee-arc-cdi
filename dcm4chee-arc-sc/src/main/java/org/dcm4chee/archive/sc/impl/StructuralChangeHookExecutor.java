        ...
        try {
            for (StructuralChangeTransactionHook scHook : structuralChangeHooks) {
                if (!scHook.beforeCommitStructuralChanges(changeContext)) {
                    LOG.info("Structural change hook {} marked transaction as failed", 
                            scHook.getClass().getName());
                }
            }
        } catch (Exception e) {
            LOG.error("Error while executing structural change hook in before-commit phase", e);
            return false;
        }
